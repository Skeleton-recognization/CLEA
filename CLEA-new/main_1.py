...
def train():
    ...
    logger_path = os.path.join(Config().output_dir, 'NB_{}_{}.log'.format(Config().dataset, Config().log_fire))
    ...
    try:
        ...
        if Config().before_epoch > 0:
            PATH = os.path.join(Config().MODEL_DIR, "base_model_{}_{}.pt".format(Config().dataset,
                                                                                 'basemodel'))
                    ...
                    if (hit_ratio > best_hit_ratio) | (f1 > best_f1) | (ndcg > best_ndcg):
                        ...
                        if better > 1:
                            model_name = os.path.join(Config().MODEL_DIR,
                                                      "base_model_{}_{}.pt".format(Config().dataset,
                                                                                           Config().log_fire))
                            checkpoint = {'epoch': epoch,
                                          'model_state_dict': NB.state_dict(),
                                          # 'optimizer2_state_dict': optimizer_dict[2].state_dict(),
                                          # 'optimizer1_state_dict': optimizer_dict[1].state_dict(),
                                          # 'optimizer0_state_dict': optimizer_dict[0].state_dict()
                                          }
                ...
                if (hit_ratio > best_hit_ratio) | (recall > best_recall) | (precision > best_precision) | (
                    ...
                    if better > 1:
                        best_PATH = os.path.join(Config().MODEL_DIR,
                                                 "model_1_{}_{}.pt".format(Config().dataset,
                                                                                               Config().log_fire))
                        checkpoint = {'epoch': epoch,
                                      'temp0': temp0,
                                      'G1_flag':G1_flag,
